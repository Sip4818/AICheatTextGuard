...
class BasicFeatureGenerator(BaseEstimator, TransformerMixin):
    ...
    def get_cleaned_string(self, text):
        if not isinstance(text, str): return ""
        # This is already a very strong cleaning method!
        # It handles lowercasing, non-alphanumeric removal, and whitespace normalization.
        text = text.lower()
        text = re.sub(r"[^\w\s]", "", text)
        return " ".join(text.split())
    ...
    def transform(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        # --- CRITICAL FIX START ---
        # We clean the text immediately to remove trailing spaces and \xa0 chars.
        # This prevents the model from "cheating" with character_count.
        df["text"] = df["text"].apply(lambda x: " ".join(str(x).split()))
        # --- CRITICAL FIX END ---

        # 1. Generate Cleaned Text (For the Embedding stage)
        # Since we already split/joined above, this is now extra safe.
        df["cleaned_text"] = [self.get_cleaned_string(t) for t in df["text"]]

        # 2. Vectorized Pandas Operations for Basic Stats
        # These will now return the REAL values without the invisible leakage.
        df["text_character_count"] = df["text"].str.len()
        df["text_word_count"] = df["text"].str.split().str.len()
