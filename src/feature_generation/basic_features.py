...
class BasicFeatureGenerator(BaseEstimator, TransformerMixin):
    ...
    def get_capital_words_count(self, text):
        words = text.split()
        return sum(
            1 for word in words if word.isupper() and any(c.isalpha() for c in word)
        )
    ...
    def transform(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        df["text_punctuation_count"] = [
            self.get_punctuation_count(t) for t in text_list
        ]
        df["text_avg_word_length"] = [self.get_avg_word_length(t) for t in text_list]
        df["text_capital_words_count"] = [
            self.get_capital_words_count(t) for t in text_list
        ]

        # Regex-based features
        df["text_number_count"] = [
            len(self.number_pattern.findall(t)) for t in text_list
        ]
        df["text_symbol_count"] = [
            len(self.symbol_pattern.findall(t)) for t in text_list
        ]
