...
class ModelTrainer:
    ...
    def _log_metrics(
        self, fold_idx: Any, y_true: np.ndarray, y_prob: np.ndarray, prefix: str = ""
    ) -> float:
    ...
    def _generate_oof_xgb_preds(
        self, X: np.ndarray, y: np.ndarray, best_params_xgb_lvl1: dict
    ) -> np.ndarray:
        try:
            ...
            save_numpy(
                oof_preds_xgb, self.model_trainer_config.xgb_level1_oof_predictions_path
            )
            assert_file_exists(
    ...
    def _generate_oof_lr_preds(
        self, X: np.ndarray, y: np.ndarray, best_params_lr_lvl1: dict
    ) -> np.ndarray:
        try:
            N_FOLDS = self.model_trainer_config.folds
            skf = StratifiedKFold(n_splits=N_FOLDS, shuffle=True, random_state=SEED)
            ...
            save_numpy(
                oof_preds_lr, self.model_trainer_config.lr_level1_oof_predictions_path
            )
            assert_file_exists(
                self.model_trainer_config.lr_level1_oof_predictions_path, "OOF lr numpy"
            )
            log_file_size(
                self.model_trainer_config.lr_level1_oof_predictions_path, "OOF lr numpy"
            )
    ...
    def train(
        ...
        try:
            ...
            save_object(
                lr_lvl1_pipeline, self.model_trainer_config.lr_level1_model_path
            )
    ...
    def initiate_model_training(self) -> ModelTrainerArtifact:
        try:
            ...
            if self.model_trainer_config.enable_tuning:
                ...
                params_dict_format["model_trainer"]["level1"]["lr"] = (
                    lvl1_lr_params_cleaned
                )
                params_dict_format["model_trainer"]["level1"]["xgb"] = (
                    lvl1_xgb_params_cleaned
                )
                ...
                params_dict_format["model_trainer"]["level2"]["lr"] = (
                    lvl2_lr_params_cleaned
                )
                write_yaml(params_dict_format, params_yaml_file_path)

            else:
                best_params_lr_lvl1 = asdict(
                    self.model_trainer_final_params_config.level1.lr
                )
                best_params_xgb_lvl1 = asdict(
                    self.model_trainer_final_params_config.level1.xgb
                )
                best_params_lr_lvl2 = asdict(
                    self.model_trainer_final_params_config.level2.lr
                )
            ...
            logger.info(
                f"Model trainer stage completed. Final model saved at: {self.model_trainer_config.final_model_path}"
            )
